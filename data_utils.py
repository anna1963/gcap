...
def remove_station_from_stream(data_stream,inv,bad_station_file='bad-quality-station.txt',verbose=True):
    if not os.path.isfile(bad_station_file):
        sys.exit('No bad_quality_station_file: '+bad_station_file)#os.path.isfile is used to check whether the specified path is an existing regular file or not.
                ...
                for ch in st:
                    if len(sta_info) == 2:
                        ...
                        if nt.code == net and st.code==sta and ch.location_code == loc and ch.code[0:2] == chan1:
                            ...
                            break
        if not found:
            sys.exit('Check if '+net_sta+' exists in the inventory')#exit and print out the thing in the bracket

...
stream.sort(keys=['distance','network','station','location','channel'])#The traces will be sorted according to the keys list. It will be sorted by the first item first, then by the second and so on. It will always be sorted from low to high and from A to Z.
print(stream)
print('Total number of stations acquired: ',int(len(stream)/3))#why /3?
...
f=open(data_pkl,'wb')
pickle.dump(ev,f)#(what we want to dump, where do we what to dump on)
pickle.dump(inv,f)

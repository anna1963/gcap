...
#A brief summary of the file: 1. load data (ev, inv, str) from data.pkl;
#2. filtering str and save each qualified(based on sac_utils.stream_add_stats)
#trace into a sac file; 3. define depths(a list)(?);
#4. writing a file 'fk_bash.cmd' and run it on bash
# this code can be only run in the data directory where data.pkl sits.
data_dir=os.getcwd() #This method returns current working directory of a process.
...
#A//B, A/B/, A/./B and A/foo/../B all will be normalized to A/B.
...
os.makedirs(green_dir,exist_ok=True)#os.makedirs() method will create all unavailable/missing directory in the specified path
...
#The math.pow(x,y) method returns the value of x raised to power y.

pre_origin_length=50
record_length = 180 # 3 min

#brief summery of below codes: writing a file 'fk_bash.cmd' and run it on bash
# (if run_bash = True);
...
#glob.glob(pathname, *, recursive=False): Return a possibly-empty list of path names

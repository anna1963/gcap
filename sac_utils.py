...
# select traces from data_stream(a stream) if the trace's sta, net, and chan are
# in inv (& not missing components).
# save qualified trace into a file named (sta.code+'.'+net.code+'.'+chan.location_code+'.'+chan.code+'.sac')
            ...
            for tr in str1:
                for chan in sta:
                    if tr.stats.channel == chan.code and tr.stats.location == chan.location_code:
                        break #exit the loop--for chan in sta; make sure that all channals of all tr from str1 are in inv

                else:
                    print('Problem finding channel in inventory',tr); exit() #whay this else statement is not intended?
                ...
                #Computes the distance between two geographic points(location of the trace(chan) and location of the event) on the WGS84 ellipsoid and the forward and backward azimuths between these points.
                if write_sac==True:
                    sac= AttribDict()
                    ...
                    tr.write(tr_name,format='SAC')#Save current trace into a file

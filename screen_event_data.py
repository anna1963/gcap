...
if remove_response: 
    stream.remove_response(water_level=50,output='VEL')# Deconvolve instrument response for all Traces in Stream;other options: output = 'DISP', 'ACC'
    #water level which represents a clipping of the inverse spectrum and limits amplification to a certain maximum cut-off value (water_level in dB) #not sure why use water level 50 here?
...
if bp_filter:
    ...
    stream.detrend("linear")# Fitting a linear function to the trace with least squares and subtracting it; Remove a trend from the trace.
...
for i in range(int(len(stream)/3)):
  ...
  fig = plt.figure()
  str.plot(fig=fig,show=False)#Use an existing matplotlib figure instance
  Ptime=0; Stime=0
  for j in range(3):
      # add origin time
      ax=fig.axes[j]
      ax.axvline(date2num(ev.origins[0].time.datetime),lw=2)#add a vertical line at time = ev.origins[0].time.datetime with linewidth 2)
#      print('depth',ev.origins[0].depth/1000,'dist',str[j].stats.distance/1000./111.195)
      ...
      ## display snr(Signal-to-noise ratio)
